...
try:
    ...
    print("username/password is wrong, or neo4j service is not running. ")
    # print('账号密码错误 或 neo4j服务未开启') # 使用中文会报错....
    flag = 0
...
if flag:
    # 导入节点 电影类型  == 注意类型转换
    print('Load gener...')
    cypher = """LOAD CSV WITH HEADERS  FROM "file:///data/IMDB/genre.csv" AS line
                MERGE (p:Genre{gid:toInteger(line.genre_id),name:line.genre_name})"""
    graph.run(cypher)

    # 导入节点 演员信息
    print('Load person...')
    cypher = """LOAD CSV WITH HEADERS FROM 'file:///data/IMDB/person.csv
    ' AS line
    ...
    # 导入节点 电影信息
    print('Load movie...')
    cypher = """LOAD CSV WITH HEADERS  FROM "file:///data/IMDB/movie.csv
    " AS line    
    ...
    print('Part1...')
    cypher = """LOAD CSV WITH HEADERS FROM "file:///data/IMDB/person_to_movie_1.csv
    " AS line
    ...
    print('Part2..')
    cypher = """LOAD CSV WITH HEADERS FROM "file:///data/IMDB/person_to_movie_2.csv
    " AS line
    match (from:Person{pid:toInteger(line.person_id)}),(to:Movie{mid:toInteger(line.movie_id)})
    merge (from)-[r:actedin{pid:toInteger(line.person_id),mid:toInteger(line.movie_id)}]->(to)
    """
    res = graph.run(cypher, timeout=60000)
    print('Part3..')
    cypher = """LOAD CSV WITH HEADERS FROM "file:///data/IMDB/person_to_movie_3.csv
    " AS line
    match (from:Person{pid:toInteger(line.person_id)}),(to:Movie{mid:toInteger(line.movie_id)})
    merge (from)-[r:actedin{pid:toInteger(line.person_id),mid:toInteger(line.movie_id)}]->(to)
    """
    ...
    cypher = """LOAD CSV WITH HEADERS FROM "file:///data/IMDB/movie_to_genre.csv
    " AS line  
